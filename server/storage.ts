  ...
  async getIntelligenceOverview(): Promise<IntelligenceOverview> {
    ...
    const result = await db.execute(sql`
      SELECT
        ...
        (SELECT COUNT(*) FROM ${customers} WHERE tags @> '"vip"'::jsonb) as vip_customers,
        (SELECT COUNT(*) FROM ${customers} WHERE tags @> '"at-risk"'::jsonb) as at_risk_tagged,
        (SELECT COUNT(*) FROM ${customers} WHERE tags @> '"new"'::jsonb) as new_customers,
        (SELECT COUNT(*) FROM ${customers} WHERE tags @> '"referral-champion"'::jsonb) as referral_champions,
