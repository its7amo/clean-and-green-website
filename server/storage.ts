  ...
  async getIntelligenceOverview(): Promise<IntelligenceOverview> {
    ...
    const result = await db.execute(sql`
      SELECT
        ...
        (SELECT COUNT(*) FROM ${customers} WHERE 'vip' = ANY(COALESCE(tags, ARRAY[]::text[]))) as vip_customers,
        (SELECT COUNT(*) FROM ${customers} WHERE 'at-risk' = ANY(COALESCE(tags, ARRAY[]::text[]))) as at_risk_tagged,
        (SELECT COUNT(*) FROM ${customers} WHERE 'new' = ANY(COALESCE(tags, ARRAY[]::text[]))) as new_customers,
        (SELECT COUNT(*) FROM ${customers} WHERE 'referral-champion' = ANY(COALESCE(tags, ARRAY[]::text[]))) as referral_champions,
